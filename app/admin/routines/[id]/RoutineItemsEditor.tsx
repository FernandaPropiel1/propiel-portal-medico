...
export default function RoutineItemsEditor({ routineId, products, routineItems }) {
  const [, startTransition] = useTransition();
  const initialMap = new Map(routineItems.map((r) => [r.product_id, { note: r.step_note ?? '', order: r.sort_order }]));
  const [state, setState] = useState(() => new Map(products.map((p) => [p.id, { included: initialMap.has(p.id), note: initialMap.get(p.id)?.note ?? '', order: initialMap.get(p.id)?.order ?? 0 }])));
  const nextOrder = useMemo(() => { let max = 0; state.forEach((s) => { if (s.included && s.order > max) max = s.order; }); return max + 1; }, [state]);
  function toggle(productId) { const current = state.get(productId) ?? { included: false, note: '', order: 0 }; const included = !current.included; const order = included ? nextOrder : current.order; const next = { included, note: current.note, order }; setState(new Map(state).set(productId, next)); startTransition(() => { setRoutineItem(routineId, productId, included, next.note, order); }); }
  function updateNote(productId, note) { setState(new Map(state).set(productId, { ...(state.get(productId) ?? { included: false, note: '', order: 0 }), note })); }
  function saveNote(productId) { const current = state.get(productId); if (!current || !current.included) return; startTransition(() => { setRoutineItem(routineId, productId, true, current.note, current.order); }); }
