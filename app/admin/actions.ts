...
function splitList(value: FormDataEntryValue | null): string[] { if (!value) return []; return String(value).split(',').map((s) => s.trim()).filter(Boolean); }
function readCities(formData: FormData): string[] { const cities: string[] = []; if (formData.get('city_mty') === 'on') cities.push('MTY'); if (formData.get('city_trc') === 'on') cities.push('Torreón'); return cities.length > 0 ? cities : ['MTY', 'Torreón']; }
export type KeyIngredient = { name: string; concentration: string | null };
function parseKeyIngredients(value: FormDataEntryValue | null): KeyIngredient[] {
  if (!value) return [];
  return String(value)
    .split('\n')
    .map((line) => line.trim())
    ...
    .map((line) => {
      ...
      const concRaw = rest.join('|').trim();
      return { name, concentration: concRaw ? concRaw : null };
    })
...
export async function setCatalogItem(doctorId: string, productId: number, inCatalog: boolean, isNewLaunch: boolean) {
  const supabase = createClient();
...
export async function logSale(doctorId: string, formData: FormData) {
  const supabase = createClient();
...
export async function deleteSale(saleId: string, doctorId: string) {
  const supabase = createClient();
...
export async function setAmbassadorCatalogItem(ambassadorId: string, productId: number, inCatalog: boolean, isFeatured: boolean) {
  const supabase = createClient();
...
export async function logAmbassadorSale(ambassadorId: string, formData: FormData) {
  const supabase = createClient();
  const productIdRaw = formData.get('product_id');
...
export async function updateSeller(sellerId: string, formData: FormData) {
  const supabase = createClient();
...
export async function setPromotedProduct(productId: number, promoted: boolean, note: string) {
  const supabase = createClient();
...
export async function setRoutineItem(routineId: string, productId: number, included: boolean, stepNote: string, sortOrder: number) {
  const supabase = createClient();
...
export async function createTrainingMaterial(formData: FormData) {
  const supabase = createClient();
...
export async function deleteTrainingMaterial(id: string, storagePath: string | null) {
  const supabase = createClient();
...
export async function createSellerReferral(sellerId: string, formData: FormData) {
  const supabase = createClient();
...
export async function confirmDoctorReferralAmount(recordId: string, formData: FormData) {
  const supabase = createClient();
...
export async function updateMonthlyFeature(featureId: string, formData: FormData) {
  const supabase = createClient();
...
export async function setCurrentMonthlyFeature(featureId: string) {
  const supabase = createClient();
...
export async function deleteMonthlyFeature(featureId: string) {
  const supabase = createClient();
...
export async function addMonthlyDuo(featureId: string, formData: FormData) {
  const supabase = createClient();
...
export async function deleteMonthlyDuo(duoId: string, featureId: string) {
  const supabase = createClient();
  const { error } = await supabase.from('seller_monthly_duos').delete().eq('id', duoId);
  if (error) throw new Error(error.message);
  revalidatePath(`/admin/monthly/${featureId}`); revalidatePath('/vendedora/panel');
}
export async function addMonthlyPromotion(featureId: string, formData: FormData) {
  const supabase = createClient();
...
export async function deleteMonthlyPromotion(promoId: string, featureId: string) {
  const supabase = createClient();
  const { error } = await supabase.from('seller_monthly_promotions').delete().eq('id', promoId);
  if (error) throw new Error(error.message);
  revalidatePath(`/admin/monthly/${featureId}`); revalidatePath('/vendedora/panel');
}
export async function updateSiteCopy(key: string, value: string) {
  const supabase = createClient();
